...
For the pessimistic proofs rollups (since they are initialized with an empty genesis root), the location that should contain the genesis allocations is constructed following the pattern `./genesis/pp/<rollup_manager_alias>/allocs.json`.
**Example**
...
The genesis file should be named as `allocs.json` and placed in the `./genesis/pp/cardona/cdk-pp-1/`, so the full path is `./genesis/pp/cardona/allocs.json`.
