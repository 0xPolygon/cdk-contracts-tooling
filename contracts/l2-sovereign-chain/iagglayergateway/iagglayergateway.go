...
// IagglayergatewayUpdatePessimisticVKeyIterator is returned from FilterUpdatePessimisticVKey and is used to iterate over the raw logs and unpacked data for UpdatePessimisticVKey events raised by the Iagglayergateway contract.
type IagglayergatewayUpdatePessimisticVKeyIterator struct {
	Event *IagglayergatewayUpdatePessimisticVKey // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *IagglayergatewayUpdatePessimisticVKeyIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(IagglayergatewayUpdatePessimisticVKey)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *IagglayergatewayUpdatePessimisticVKeyIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *IagglayergatewayUpdatePessimisticVKeyIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// IagglayergatewayUpdatePessimisticVKey represents a UpdatePessimisticVKey event raised by the Iagglayergateway contract.
type IagglayergatewayUpdatePessimisticVKey struct {
	Selector           [4]byte
	Verifier           common.Address
...
func (_Iagglayergateway *IagglayergatewayFilterer) FilterUpdatePessimisticVKey(opts *bind.FilterOpts) (*IagglayergatewayUpdatePessimisticVKeyIterator, error) {

	logs, sub, err := _Iagglayergateway.contract.FilterLogs(opts, "UpdatePessimisticVKey")
	if err != nil {
		return nil, err
	}
	return &IagglayergatewayUpdatePessimisticVKeyIterator{contract: _Iagglayergateway.contract, event: "UpdatePessimisticVKey", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event UpdatePessimisticVKey(bytes4 selector, address verifier, bytes32 newPessimisticVKey)
func (_Iagglayergateway *IagglayergatewayFilterer) WatchUpdatePessimisticVKey(opts *bind.WatchOpts, sink chan<- *IagglayergatewayUpdatePessimisticVKey) (event.Subscription, error) {

	logs, sub, err := _Iagglayergateway.contract.WatchLogs(opts, "UpdatePessimisticVKey")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(IagglayergatewayUpdatePessimisticVKey)
				if err := _Iagglayergateway.contract.UnpackLog(event, "UpdatePessimisticVKey", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event UpdatePessimisticVKey(bytes4 selector, address verifier, bytes32 newPessimisticVKey)
func (_Iagglayergateway *IagglayergatewayFilterer) ParseUpdatePessimisticVKey(log types.Log) (*IagglayergatewayUpdatePessimisticVKey, error) {
	event := new(IagglayergatewayUpdatePessimisticVKey)
	if err := _Iagglayergateway.contract.UnpackLog(event, "UpdatePessimisticVKey", log); err != nil {
		return nil, err
	}
